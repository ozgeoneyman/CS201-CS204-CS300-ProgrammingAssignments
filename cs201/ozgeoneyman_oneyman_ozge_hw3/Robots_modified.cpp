...
int Robot :: GetXCoordinate() //returns x coordinate of the robot object
{
	return xPos;
}
int Robot :: GetYCoordinate() //returns y coordinate of the robot object
{
	return yPos;
}

Direction Robot :: GetDirection()	//returns the direction of the object
{
	return direction;
}

void Robot :: TurnFace(Direction direct) //turn the robot object towards this direction
{
	direction = direct;
}

void Robot :: TurnRandom() //turn the robot object towards a random direction
{
	RandGen randint;
	int RandomNum = randint.RandInt(0,3);
	if (RandomNum == 0)
	{
		direction = west;
	}
...
bool Robot :: IsAlive() // check the robot is alive or not
{
	return !stalled;
}
void Robot :: Resurrect () //resurrect the robot 
{
	if (stalled)
	{
		stalled = false;
...
void Robot :: PickAllThings () //pick all things in the current cell
{
	int cellcount = GetCellCount(xPos,yPos);
	for (int i = 0; i < cellcount; i++)
	{
		PickThing();
	}
}
void Robot :: PutManyThings () //put many things on the current cell
{
	if ((xPos==0 && yPos==0)||(xPos==0 && yPos==12)||(xPos==12 && yPos==0)||(xPos==12 && yPos==12))
	{
		int cellc= GetCellCount(xPos,yPos);
		if ((bag + cellc) <= 10)
		{
				PutThings(xPos, yPos, bag);
				bag=0;
		}
		else
		{
						
			PutThings(xPos, yPos, 10-cellc);
			bag = bag - (10-cellc);
		}
	
	}
}
void Robot :: Turn (Direction direct) //turn robot that direction
{
	if (!stalled)
	{
		direction= direct;
...
void Robot :: MonsterMove() //random move for monster robots
{
	RandGen rn;
	int m = rn.RandInt(0,3);
	if (m==0)
	{
		Turn(east);
